...
//      - Status can indicate if the appointment is "Scheduled:0", "Completed:1", or other statuses (e.g., "Canceled") as needed.
    ...
    public AppointmentDTO(Appointment appo){
        this.id = appo.getId();
        this.doctorId = appo.getDoctor().getId();
        this.doctorName = appo.getDoctor().getName();
        this.patientId = appo.getPatient().getId();
        this.patientName = appo.getPatient().getName();
        this.patientEmail = appo.getPatient().getEmail();
        this.patientPhone = appo.getPatient().getPhone();
        this.patientAddress = appo.getPatient().getAddress();
        this.appointmentTime = appo.getAppointmentTime();
        this.status = appo.getStatus();
        this.appointmentDate = appointmentTime.toLocalDate();
        this.appointmentTimeOnly = appointmentTime.toLocalTime();
        this.endTime = appointmentTime.plusHours(1);
    }
